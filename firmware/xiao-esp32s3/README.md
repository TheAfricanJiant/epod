...
| Storage | SD card — music library and recordings, hot-swappable |
| Audio in | PDM microphone (recording and inference) |
| Radios | Wi-Fi Soft-AP (uploads, signage API) and BLE (control, telemetry) |
| Link | UART line protocol to the WROVER |

Audio *output* and the user interface belong to the WROVER. Everything bursty
lives here so that playback is never interrupted by an SD seek, a radio, or an
inference pass.
...
MFE filterbank is computed in quantized int8 and the features come out
degenerate — the DSP and the network both run and report timings, and every
class still returns `0.00`. It must be a **build flag**, not a `#define` in
`main.cpp`, because the library's own `.cpp` files are separate translation
units that would never see it.
...
| UART TX → WROVER RX2 | 43 |
| UART RX ← WROVER TX2 | 44 |
...
the exact gate a word died at (`below threshold`, `too close to call`,
...
| `VOICE_MARGIN` | 0.15 | winner must lead the runner-up |
...
one slice per call, so it must be fed every slice in order — skipping quiet
slices splices non-adjacent audio into the window and corrupts the very features
a spoken word depends on.

### Microphone gain

The voice path reproduces the conditions the training set was captured under:
hardware `amplify_num` at the driver default of 1, then a ×4 software shift.
The recorder keeps its own higher gain. Both are applied by
`micConfigure(rate, amplify)`, which re-clocks and re-slots the single PDM
channel on the way in and out of each mode.
...
**each switches the other off first**. Enabling vision also stops playback: a
320×320 frame plus inference leaves no room to stream audio off the card, and
...
```
   camera watching an empty table
              ...
              ▼
   onGuestDetected()
     ├─ setVisionMode(false)      camera stands down
     ├─ webEmit("person")         eyes open on the web app
     ├─ requestPlay()             ambient music
     └─ setVoiceMode(true)        menu keywords take over
```

`onGuestGone()` reverses it. Both modes are also exposed in the WROVER's
settings menu so either can be killed by hand.

---

## Web API

Served over the Soft-AP once Wi-Fi is on. CORS headers are sent, so a browser
can be pointed straight at the device if the laptop has no Python.
...
`seq` increments on every new event. The web app acts whenever it changes, so an
event is never missed between two polls and never replayed after a page reload.
...
| Command | Effect |
|---|---|
| `VOICE` | toggle per-inference reporting, print the model's class list |
...
one library without an Enterprise plan. `tools/merge_impulse.py` does it by hand:
...
`EI_CLASSIFIER_*` macros — so `model_metadata.h`, the one file that genuinely
...
2. rewrites `trained_model_ops_define.h` as the **intersection** of both DISABLE
   ...
   kernel the other model needs, and that surfaces at runtime as a model which
...
against different TensorFlow Lite Micro generations — add a forwarder beside the
one in `src/tflite-model/ei_merge_shim.cpp`.

---

## Vision status

The person-detection path is **not currently running on-device**. The merge
itself succeeds and links, but the available export is MobileNet SSD at 320×320:
...
It compiles to roughly **12.8 MB against a 3.34 MB partition**. The architecture
is meant for a Raspberry Pi or a phone, not an MCU.

**The fix is in Edge Impulse Studio:** change the object-detection block to
**FOMO** at 96×96 grayscale — typically 50–100 KB — and re-export. The existing
training images are reusable; only the learn block changes. Everything
downstream (`onGuestDetected()`, mode arbitration, the web handover) is built
and tested, waiting on that one seam.
...
| Symptom | Cause |
|---|---|
| Every class returns `0.00`, `nn` non-zero | `EIDSP_QUANTIZE_FILTERBANK` not 0, or a performance-calibration block in the impulse suppressing output until it detects an event |
