...
// MERGED: voice keyword spotting + person detection.
// A DISABLE is kept only when BOTH models disable that kernel. Taking the
// union instead would compile out a kernel the other model needs, and the
// failure shows up at runtime as a model that loads and returns nothing.
// 68 kept, 12 re-enabled because one of the two models uses them.
